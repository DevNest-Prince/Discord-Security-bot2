...
const raidHandler = require('./interactions/raidHandler');
const selfRoleHandler = require('./interactions/selfRoleHandler');
...
module.exports = {
  ...
  async execute(interaction) {
    if (interaction.isChatInputCommand()) {
      ...
      if (isAdminLockEnabled() && !config.isOwner(interaction.user.id)) {
      ...
      if ((await getCachedIgnore(`cmdLock:${interaction.commandName}`, () => commandLockDb.isLocked(interaction.commandName))) && !config.isOwner(interaction.user.id)) {
        const lockContainer = new ContainerBuilder()
